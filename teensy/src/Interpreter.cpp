...
Interpreter::Interpreter(Stream& _serial, char* _token): serial(_serial), token(_token)
{
    input.index = 0;
    commands.index = 0;
...
bool Interpreter::add(Command& command)
{
...
bool Interpreter::add(char* command, void (*callback)(char*))
{
...
bool Interpreter::remove(Command& command)
{
    for (int i = 0; i < this->commands.index; i++) {
        if (this->commands.items[i] == &command) {
...
#ifdef COMMANDLINE_PRE_POST
    void Interpreter::attachPre(void (*callback)(char*))
    {
        this->preCallback = callback;
    }

    void Interpreter::attachPost(void (*callback)(char*, bool))
    {
